...
expr_to_struct_str(Ex,""):-retractall(glob_string(_)),translate_struct(Ex,Text,0), %Этот предикат выдает структуру expr-выражения
...
otstup(0,Ot,Ot):-!. %Предикат строит строку из N пробелов
otstup(N,Ot,OtRes):- concat(" ",Ot,Ot1), 
		N1=N-1,
		otstup(N1,Ot1,OtRes).

translate_struct(nc(Word),Res,N):-!, %Этот предикат строит структуру expr-выражения
	otstup(N,"",Ot),
	format(Res,"%<L%>\"%\"</L%>\n",Ot,N,Word,N).
...
translate_struct(undef,Res,N):-!,
	otstup(N,"",Ot),
	format(Res,"%<L%>%</L%>\n",Ot,N,"Выражение не определено",N).
...
translate_struct(_,Res,N):-
	otstup(N,"",Ot),
	format(Res,"%<L%>Выражение не распознано</L%>\n",Ot,N,N).
   
translate_struct_list([],[],_):-!.  %Предикат строит по списку expr- выражений список структур этих выражений с отступом длины N 
...
form_intemplate(DescLeft,Rule, DescNt):-
	...
	not(Tok="'"), %% не начинается с одинарной кавычки
...
form_intemplate(DescLeft,Rule, DescNt):-       %% общий случай
	...
	form_intemplate_list(DescNt,Rule,[],List), %% новый предикат для формирования списка  слов и переменных шаблона п
							%% по тексту Rule
	...
	assert(in_template(1,IdC,Id,Text,Op,"Шаблон грамматики", List, DescLeft)).
...
form_intemplate_list(DescNt,Rule,InList,ResList):-
	send_to_debug("Дошел до правила с' "),
	fronttoken(Rule,Tok1,Rest1),
	Tok1="'",
	fronttoken(Rest1,Tok2,Rest2),
	send_to_debug(Tok2),
	...
	send_to_debug(Tok3),
	Tok3="'",!, %% через слово после кавычки стоит одинарная кавычка	
	send_to_debug(RestRule),
	append(InList,[w(Tok2)],InListNew), %% вводится терминал в список
	form_intemplate_list(DescNt, RestRule, InListNew, ResList).
	
	
	
form_intemplate_list(DescNt, Rule,InList,ResList):-
	fronttoken(Rule,Tok,RestRule),!, %% вводится нетерминал Tok
	create_new_constant(nc(Tok), DescNt,DescTok),
